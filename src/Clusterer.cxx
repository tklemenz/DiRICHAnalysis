  ...
  if (particleType == ParticleType::Cosmic) {
    ...
    for (auto& module : modules) {
      for (auto& fiber : module.getFibers()) {
        if (fiber.getNSignals() == 0) { continue; }
    ...
    /// The assumption for cosmics data that per event and layer there can only one proper cluster and not more cannot be used.
    /// Lots of background radiation in the lab, so more than one cluster can be within one layer.
    for (auto& layer : signalsVec) {                                                        /// loop over signals layer by layer
      ...
      for (auto& signal : layer) {
        if (signal.getToT() >= tot_lowercut && signal.getToT() <= tot_uppercut) { goodToTInLayer++; }
      }
      if (debug) {
      ...
      while (goodToTInLayer > 0) {                                                          /// look for clusters with ToT max > tot_lowercut
        if (debug) { printf("%s%s %sLooking for Cluster... %d good signal(s) to be found...%s\n",text::LBLU, process.c_str(), text::CYN, goodToTInLayer, text::RESET); }
        Double_t maxToT = 0;
        ULong_t index = -1;
